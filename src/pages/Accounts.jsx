...
const Accounts = () => {
  ...
  const isAddModalOpen = userStore((state) => state.isAddModalOpen)
  const toggleAddModal = userStore((state) => state.toggleAddModal)
  ...
  const message = userStore((state) => state.message)
  ...
  return (
    <div className="accounts-wrapper">
     {message && status ? <Toaster message={message} status={status}/> : ''} 
      ...
      />
      <AddModal open={isAddModalOpen} handleClose={toggleAddAccountModal} />
      <EditModal
      ...
      {!loading ? (
        <Box sx={{ width: "100%" }}>
          <Paper sx={{ width: "100%", mb: 2 }}>
            <TableToolbar handleSearch={handleSearch} openModal={toggleAddAccountModal} />
            <TableContainer>
              <Table
                sx={{ minWidth: 750 }}
